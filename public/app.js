...
const COOLDOWN_MS = 10000; // 10s cooldown before re-scanning same face
...
async function tryMarkAttendance(descriptor) {
  ...
  // Client-side cooldown to avoid spamming server
  const now = Date.now();
  ...
  try {
    ...
    if (!res.ok) {
      if (data.error === 'Face not recognized') {
        statusEl.textContent = `Face not recognized (dist: ${data.distance ? data.distance.toFixed(2) : '?'}). Is this worker enrolled?`;
    ...
    statusEl.textContent = `${data.record ? data.record.name : ''} — done ✔`;
  } catch (e) {
...
document.getElementById('captureBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (!detection) {
      status.textContent = 'No face detected. Face the camera with good lighting.';
      return;
    }
...
document.getElementById('enrollForm').addEventListener('submit', async (e) => {
  ...
  try {
    const res = await fetch('/api/enroll', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: empName, employeeId: empId, descriptor: capturedDescriptor })
    });
    const data = await res.json();
    ...
    workers.forEach(w => {
      ...
      delBtn.onclick = async () => {
        if (confirm(`Remove ${w.name}?`)) {
          await fetch(`/api/workers/${w.employeeId}`, { method: 'DELETE' });
          loadWorkerList();
        }
      };
...
document.getElementById('exportDaily').addEventListener('click', () => {
  ...
  window.open(`/api/export/daily?date=${date}`, '_blank');
});
...
async function loadDailyReport(date) {
  ...
  try {
    ...
    if (data.report.length === 0) {
      tbody.innerHTML = '<tr><td colspan="5">No workers enrolled.</td></tr>';
      return;
    }
...
async function loadMonthlyReport() {
  ...
  try {
    ...
    if (data.summary.length === 0) {
      tbody.innerHTML = '<tr><td colspan="6">No workers enrolled.</td></tr>';
      return;
    }
    ...
    data.summary.forEach((w, i) => {
      const pct = data.workingDays > 0
        ? ((w.totalPresent / data.workingDays) * 100).toFixed(0)
        : '0';
      const tr = document.createElement('tr');
      ...
      tr.innerHTML = `
        ...
        <td>${w.employeeId}</td>
        <td>${w.name}</td>
        <td><strong>${w.totalPresent}</strong></td>
